program get_dipdir
  ...
  if (me==0) print*, 'unnorm, nbj, dl,nlbands, nbins, nell,nside_spots,nsim,nspots',unnorm,nbj,2*dl, nlbands, nbins, nell,nside_spots,nsim,nspots
...
!   endif
  ...
  do kk=0,nsim_pp(me)-1
        ...
        do i=0,nlbands-1 
           ...
           do zz = b1,min(b2,nbins-1)
              if (sim == 0) then  !this is the data
                 ...
                 if (unnorm .eq. 2) mres_aps=Aps/pp(bins(zz))**2 !unnorm=2 implies no mean subtraction


                 if (unnorm.eq.3) then
                    diff_mean = diff_mean + (res(zz,j) - mres_aps)/dble(band_width)/mres_aps
                    var_sum = var_sum + (dres(zz,j)/mres_aps)**2 
                 else
                 ...
                 if (unnorm .eq. 2) mres_aps=0.

                 if (unnorm.eq.3) then
                    diff_mean = diff_mean + (mcres(zz,j,sim-1)-mres_aps)/dble(band_width)/mres_aps 
                    var_sum = var_sum + (dres(zz,j)/mres_aps)**2 
                 else
           ...
           if (unnorm .gt. 0) bres(i,j) = diff_mean
     ...
     do i=0,nlbands-1 
        ...
        !get dipole  
        mm = bres(i,:)
        !print*,'isim, iband, sum map',sim,i,sum(mm)
        ...
        if (unnorm.gt.0) then
           ...
           !if ((unnorm.eq.3).and.(sim==0)) print*,'unnorm=3,sim=0: iband, sum map, damp, multipoles',i,sum(mm), sqrt(sum(multipoles(1:3)**2)),multipoles
           if (sim==0) print*,'sim, iband, sum, damp, multipoles',sim, i, sum(mm),sqrt(sum(multipoles(1:3)**2)),multipoles
