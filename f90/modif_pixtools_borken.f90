...
!#endif
       ...
       if (my_ahi >= my_alow) then
          ...
          do irad=1,nrad-1
             ...
             if ( (ir>=sjr_min) .and. (ir<=sjr_max) ) then !height check                
                ...
                ! nprad=pr_ahi-pr_alow+1

                ! if (ir>=1 .and. ir<=57 .and. proc_number==8 .and. irad==1) print*,'big',irad,my_alow,my_ahi, np
    ...
    !if (proc_number==8 .and. (ip == modulo(pr_low + ismall, nr))) print*,'ir,i,ismall,my_low,pr_low',ir,i,ismall,my_low,pr_low 
