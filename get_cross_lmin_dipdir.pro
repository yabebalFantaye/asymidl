pro get_cross_lmin_dipdir,bins,res,dres,mcres,mres,dipole_nside=dipole_nside,lmax=lmax,$
...
if not keyword_set(nbj) then nbj = nnbin*nband-1
...
if not keyword_set(load) then begin
    spawn,'pwd',dir
    str='cd '+dir+'; mpirun -np '+strtrim(string(nproc),2)+' -hostfile $HOME/mpihosts --prefix $MPI_HOME $HOME/software/Healpix_2.15/bin/get_cross_lmin_dipdir'
    print, '************************* spawning **********************'
    print, '** --', str, ' -- **'
    print, '*********************************************************'
    SPAWN,str
