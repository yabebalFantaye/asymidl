function make_disk, th, phi,val,rrad,nside,square=square,triangle=triangle,bad_value=bad_value,astro=astro
;;
...
nn = n_elements(th)
if n_params() lt 2 then val = findgen(nn) ;bins=dindgen(50)*16d0+10d0
if n_params() lt 3 then rrad=3d0
   ...
   rad = radvec[i]
    ...
    if not keyword_set(square) or not keyword_set(triangle) then begin
       ...
       if keyword_set(astro) then begin
          if (th(i) lt !pi/9d0) and (th(i) gt -!pi/9d0) then rad=rad  ;;±20deg increase rad
          ANG2VEC,  (!pi/2d0-th(i))*!rtod, phi(i)*!rtod, vec,/astro
       endif else begin
          if (th(i) gt !pi/9d0) and (th(i) lt 11d0*!pi/18d0) then rad=rad ;;±20deg increase rad
