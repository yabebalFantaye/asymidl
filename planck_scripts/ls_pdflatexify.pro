...
; 
;-
pro LS_pdflatexify, filename, tag, tex, scale, outname=outname, keep=keep,$
  ...
  for i=0, n_elements(tag)-1 do $
        printf, lun,'\psfrag{'+tag[i]+'}[cc][cc]['+scale[i]+']{\textit{'+tex[i]+'}}'
  ...
  free_lun, lun
  ;
  spawn, 'cd '+path+' && pdflatex '+name+'.tex', msg0
  spawn,'cd '+path+' && mv '+name+'.pdf '+outname,msg01
