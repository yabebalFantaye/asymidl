PRO planck_mollview,fname,min=min,max=max,hfi_ct=hfi_ct,ctitle=ctitle,feps=feps,small=small,medium=medium,large=large,_extra=extra
  ...
  if size(fname,/type) eq 7 then begin                              ;
     READ_FITS_MAP, fname, map, hdr, ehdr, NSIDE=NS, COORDSYS=COORD, ORDERING=ORDER, SILENT=SILENT
  endif else map=fname
                                ;
  map = REFORM(map[*,0])    ; the first [*,0] entry is the temperature/intensity portion. now in uK instead of mK.
                                ;
  Npts = N_ELEMENTS(map)
               
                 ;
  MINVAL = min(map)                 ;; The minimum value to plot is -1000 uK
  MAXVAL =  max(map)                ;; The maximum value to plot is  1000 uK
  if keyword_set(min) then MINVAL=min
  if keyword_set(max) then MAXVAL=max
                                ;
  ...
  ;if keyword_set(hfi_ct) then begin
     ...
     hfi_ct, CTDIR=CTDIR, CTFILE=CTFILE, /LOAD 
  ;endif
;; this tells me the location of the revised colour table, file CTFILE in directory CTDIR, 
  ...
  if keyword_set(small) then begin                                ;
     ...
     W   =  W_88
     Nm  = Nm_88
     LS_mollview, map, COLT=41, CTDIR=CTDIR, CTFILE=CTFILE, MIN=MINVAL, MAX=MAXVAL, CHARSIZE=1,GRATICULE=GR, GLSIZE=1,HXSIZE=W, FLIP=1, $
                  GRMIN=[-179, -89], GRMAX=[179,89], GRLS =1, OUTLINE=out, TITLE=' ', PXSIZE=PX, PS=feps, CBLBL=ctitle,_extra=extra ;, , UNIT='!Xu'  
     ...
     if keyword_set(feps) then begin
        ...
        LS_latexify, feps, IDLstrs, TEXstrs, FNTszs*8d/11d, outname=outname1+'.eps', height=SZ/10d/1.6d, width=SZ/10d, FDIR=FDIR ;, /full
                                ;
  ...
  if keyword_set(medium) then begin
     ...
     LS_mollview, map, /NESTED, COLT=41, CTDIR=CTDIR, CTFILE=CTFILE, MIN=MINVAL, MAX=MAXVAL, CHARSIZE=1,GRATICULE=GR, GLSIZE=1,HXSIZE=W, FLIP=1, $
                  GRMIN=[-179, -89], GRMAX=[179,89], GRLS =1, OUTLINE=out, TITLE=' ', PXSIZE=PX, PS=feps, CBLBL=ctitle ;, , UNIT='!Xu'  
